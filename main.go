...
func run(args []string) error {
	...
	if downloaded {
		outcome, err := checksum.VerifyAsset(downloader, rel.Assets, asset, assetPath)
		if err != nil {
			return err
		}
		if outcome == checksum.Unlisted {
			fmt.Fprintf(os.Stderr, "gh-runx: warning: %s not listed in checksum manifest; skipping verification\n", asset.Name)
		}
	}
